...
export async function onRequestPut(context) {
  ...
  try {
    ...
    const allowBookmarks = body.allow_bookmarks ? 1 : 0;

    const batchStmts = [
      env.NAV_DB.prepare('UPDATE category SET catelog = ?, sort_order = ?, parent_id = ?, is_private = ?, allow_bookmarks = ? WHERE id = ?')
        .bind(catelog, sort_order, parentId, isPrivate, allowBookmarks, categoryId),
