...
export async function onRequestGet(context) {
  ...
  try {
    ...
    const { results } = await env.NAV_DB.prepare(`
        SELECT c.id, c.catelog, c.sort_order, c.parent_id, c.is_private, c.allow_bookmarks, COUNT(s.id) AS site_count
        FROM category c
        ${siteJoin}
        ${categoryFilter}
        GROUP BY c.id, c.catelog, c.sort_order, c.parent_id, c.is_private, c.allow_bookmarks
