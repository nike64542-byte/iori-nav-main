...
function _renderHorizontalItems(cats, currentCatalogName, level) {
    return cats.map(cat => {
        ...
        const canAddBookmarks = cat.allow_bookmarks === 1;
        // 父分类只要有子分类就渲染下拉箭头和下拉菜单（始终作为可容纳子分类的目录）
        ...
        // 父分类有子分类但不允许添加书签：仅作为下拉目录，点击名称切换展开/收起（不跳转）
        if (hasChildren && !canAddBookmarks) {
    ...
    return cats.map(cat => {
        ...
        if (hasChildren) {
            ...
            <div class="flex items-center w-full">
                ...
                <button type="button" class="sidebar-cat-toggle shrink-0 px-3 py-2 flex items-center text-gray-400 dark:text-gray-500 hover:text-primary-600 dark:hover:text-primary-400" data-toggle-for="${cat.id}" aria-expanded="true" aria-label="展开或收起 ${safeName}" title="展开或收起">
                    <svg class="${chevronClass} sidebar-cat-chevron"><use href="#icon-chevron-down"/></svg>
                </button>
            </div>`;
            } else {
                // 父分类不允许添加书签：整行作为折叠目录，点击展开/收起子分类，不跳转页面
                rowHtml = `
            <button type="button" class="${baseClass} ${activeClass} sidebar-cat-toggle-name" data-toggle-for="${cat.id}" aria-expanded="true" style="padding-left: ${padLeft}">
                <svg class="h-5 w-5 mr-2 ${iconClass}"><use href="#icon-folder"/></svg>
                ${safeName}
                <svg class="${chevronClass} sidebar-cat-chevron ml-auto"><use href="#icon-chevron-down"/></svg>
            </button>`;
            }
            const childrenHtml = `
            <div class="sidebar-cat-children" data-children-for="${cat.id}">
                ${_renderVerticalItems(cat.children, currentCatalogName, isCustomWallpaper, level + 1)}
            </div>`;
            return `<div class="sidebar-cat-item" data-catid="${cat.id}">
                ${rowHtml}
                ${childrenHtml}
            </div>`;
        }
        ...
        return `
      <a href="?catalog=${catalogParam}" data-id="${cat.id}" class="${baseClass} ${activeClass}" style="padding-left: ${padLeft}">
          <svg class="h-5 w-5 mr-2 ${iconClass}"><use href="#icon-folder"/></svg>
          ${safeName}
      </a>`;
    }).join('');
