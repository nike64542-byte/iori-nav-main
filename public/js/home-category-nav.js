  ...
  Home.initCategoryNavigation = function () {
    ...
    if (sidebarEl) {
      sidebarEl.addEventListener('click', (e) => {
        const toggle = e.target.closest('.sidebar-cat-toggle, .sidebar-cat-toggle-name');
        if (!toggle) return;
        e.preventDefault();
        e.stopPropagation();
        const catId = toggle.getAttribute('data-toggle-for');
        if (!catId) return;
        const item = toggle.closest('.sidebar-cat-item');
        if (!item) return;
    ...
    // 水平菜单父分类点击切换下拉（allow_bookmarks=0 时不跳转，仅展开/收起）
    if (navContainer) {
      navContainer.addEventListener('click', (e) => {
        const toggle = e.target.closest('.horizontal-cat-toggle');
        if (!toggle) return;
        e.preventDefault();
        e.stopPropagation();
        const catId = toggle.getAttribute('data-toggle-for');
        ...
        if (isShow) {
          ...
          document.querySelectorAll('.menu-item-wrapper > .dropdown-menu.show').forEach(m => m.classList.remove('show'));
    ...
    // 点击其他区域关闭水平分类下拉
    document.addEventListener('click', (e) => {
      if (!e.target.closest('.horizontal-cat-toggle')) {
        document.querySelectorAll('.menu-item-wrapper > .dropdown-menu.show').forEach(m => m.classList.remove('show'));
      }
    });
