...
if (editCategoryForm) {
    editCategoryForm.addEventListener('submit', function (e) {
        ...
        const allowBookmarks = document.getElementById('editCategoryAllowBookmarks').checked;
        ...
        // Check duplicate name (excluding self, only within same parent)
        const isDuplicate = window.categoriesData.some(category =>
            category.catelog === categoryName &&
            category.id != id &&
            String(category.parent_id || 0) === String(parentId || 0)
        );
        if (isDuplicate) {
            window.showMessage('该分类名称在当前父分类下已存在', 'error');
            return;
        }
...
if (addCategoryForm) {
    addCategoryForm.addEventListener('submit', function (e) {
        ...
        const allowBookmarks = document.getElementById('newCategoryAllowBookmarks').checked;
